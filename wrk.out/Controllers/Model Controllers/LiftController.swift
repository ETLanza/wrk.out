...
class LiftController {
    ...
    func add(lifts: [Lift], toWorkout workout: Workout, completion: @escaping (Bool) -> Void) {
        let workoutReference = CKReference(recordID: workout.ckRecordID, action: .deleteSelf)
        var liftRecords: [CKRecord] = []
        lifts.forEach { (lift) in
            let newLift = Lift(name: lift.name, workoutReference: workoutReference, routineReference: nil)
            let record = CKRecord(lift: newLift)
            liftRecords.append(record)
        }
        CloudKitManager.shared.saveRecords(liftRecords, database: CloudKitManager.shared.privateDatabase, perRecordCompletion: { (record, error) in
            if let error = error {
                NSLog("Error saving lift to CloudKit", error.localizedDescription)
                completion(false)
                return
            ...
            guard let record = record,
                let liftFromRecord = Lift(ckRecord: record) else {
                    NSLog("Error creating lift from CKRecord")
                    completion(false)
                    return
            }
            workout.lifts.append(liftFromRecord)
